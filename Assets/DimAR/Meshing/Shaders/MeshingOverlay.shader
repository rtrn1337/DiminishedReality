Shader "Custom/MeshingOverlay" {
		...
		Pass
	{
		...
		float3 doMaterial(in float3 pos, in float3 midPos)
		{
			...
			d /= _RangeScale;
			float border = fmod(_Time.y * _Speed * _Pulse, 5.0);
			 if(_Pulse == 0)
			 {
				 border = fmod(_Speed, 5.0);
			 }
			...
			c += c2 * smoothstep(border-0.1, border - 0.6, d);
			// Cutoff front
		...
		{
			return float4(doMaterial(i.world.xyz, _Center.xyz), _Alpha);
		}
			ENDCG
		}
		}
