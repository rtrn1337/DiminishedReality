...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    public class MeshClassificationFracking : MonoBehaviour
    {
        ...
        { 
            var meshSubsystem = m_MeshManager.subsystem as XRMeshSubsystem;
            ...
            using (faceClassifications)
            {
                ...
                for (int i = 0; i < k_NumClassifications; ++i)
                {
                    ...
                    if (classifiedMeshFilter != null)
                    {
                        ...
                        ExtractClassifiedMesh(baseMesh, faceClassifications, (ARMeshClassification)i, classifiedMesh);
                        meshFilters[i].mesh = classifiedMesh;
                      //  meshFilters[i].gameObject.GetComponent<MeshCollider>().cookingOptions = MeshColliderCookingOptions.;
                       // meshFilters[i].gameObject.GetComponent<MeshCollider>().sharedMesh = null;
                        meshFilters[i].gameObject.GetComponent<MeshCollider>().sharedMesh = meshFilters[i].mesh;
